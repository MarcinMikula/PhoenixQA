"""
locator_resolution_collector.py

Gathers context for FailureCategory.LOCATOR_RESOLUTION. Moved here
unchanged from context_collector.py's original single-class shape — see
LEARNINGS.md "Sprint 6 pre-coding" Decision #2 and "Sprint 6B
(implementation)" for confirmation this was a pure move, not a rewrite
(same tests, same assertions, still passing).
...
class LocatorResolutionCollector(BaseContextCollector):
    def collect(
        self,
        broken_selector: str,
        error: Exception,
