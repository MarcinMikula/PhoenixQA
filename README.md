...
| Sprint 8  | Narrow baseline slice (Gap #9, decided) — `HeuristicProvider` (`LOCATOR_RESOLUTION`, fuzzy matching) + `PolicyOnlyProvider` (`ACTIONABILITY`/`VISIBLE`, decides off `collector_metadata` directly) vs. `llama3.2`, small-scale, deliberately BEFORE the full multi-level Benchmark Runner. Sequenced ahead of Sprint 6C-D — see `LEARNINGS.md` "Sprint 8 (pre-coding)" | 🚧 Both providers + comparison tool (`scripts/compare_baselines.py`) built and unit-verified (163/163); the actual live run against `llama3.2` has not happened yet |
