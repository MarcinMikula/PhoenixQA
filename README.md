...
| `not_visible` / `timeout_waiting` → `FailureCategory.ACTIONABILITY` (5 `ActionabilityReason` values) | 🚧 In progress — `RECEIVES_EVENTS` and `VISIBLE` fully implemented, unit-tested, AND live-verified (both directions of the policy guardrail — see below); `ENABLED`/`EDITABLE` not started; `STABLE` blocked on a deterministic Chaos App mechanism. **`Healer` now EXECUTES `WAIT_AND_RETRY`/`NO_SAFE_RECOVERY` for BOTH `VISIBLE` (live-verified, twice) and `RECEIVES_EVENTS` (unit-verified, not yet live-verified) — Sprint 8, Option A narrowed then widened. `RECEIVES_EVENTS`' `DISMISS_BLOCKER` remains proposal-only, still rejected (no DOM-validated guardrail exists for it yet).** |
...
| Sprint 6C-D | Remaining `ActionabilityReason` slices (`ENABLED`/`EDITABLE`), OR execute a policy-approved `ActionabilityStrategy` (Option A from Gap #12) | ⏸️ Deliberately sequenced AFTER the Sprint 8 baseline slice below — neither started yet, see `docs/gaps.md` Gap #9/#12 |
| Sprint 7  | Healing History — SQLite store, decision log, healing correctness definition | ⏳ Planned  |
| Sprint 8  | Narrow baseline slice (Gap #9, decided) — `HeuristicProvider` (`LOCATOR_RESOLUTION`, fuzzy matching) + `PolicyOnlyProvider` (`ACTIONABILITY`/`VISIBLE`, decides off `collector_metadata` directly) vs. `llama3.2`, small-scale, deliberately BEFORE the full multi-level Benchmark Runner. Sequenced ahead of Sprint 6C-D — see `LEARNINGS.md` "Sprint 8 (pre-coding)" | ✅ Complete — 8/8 across `LOW`/`MEDIUM`, baseline matched `llama3.2` every time. `ticket_row_ambiguity` near-tie idea investigated and deprioritized (Gap #16) |
| Sprint 8, Option A (narrowed) | `Healer` executes an approved `ActionabilityStrategy` — scoped to `VISIBLE`'s `WAIT_AND_RETRY`/`NO_SAFE_RECOVERY` only, `RECEIVES_EVENTS` untouched | ✅ Implemented, unit-tested (185/185) — live verification against a real Chaos App still pending |
