...
| Sprint 8  | Narrow baseline slice (Gap #9, decided) — `HeuristicProvider` (`LOCATOR_RESOLUTION`, fuzzy matching) + `PolicyOnlyProvider` (`ACTIONABILITY`/`VISIBLE`, decides off `collector_metadata` directly) vs. `llama3.2`, small-scale, deliberately BEFORE the full multi-level Benchmark Runner. Sequenced ahead of Sprint 6C-D — see `LEARNINGS.md` "Sprint 8 (pre-coding)" | ✅ Complete — all 3 live comparisons run, baseline matched `llama3.2` 5/5. Next decision (not yet made): narrow `VISIBLE` execution vs. more `locator_resolution` baseline evidence at `MEDIUM`/`HIGH` |
