...
| `not_visible` / `timeout_waiting` → `FailureCategory.ACTIONABILITY` (5 `ActionabilityReason` values) | 🚧 In progress — `RECEIVES_EVENTS` and `VISIBLE` fully implemented, unit-tested, AND live-verified (both directions of the policy guardrail — see below); `ENABLED`/`EDITABLE` not started; `STABLE` blocked on a deterministic Chaos App mechanism. **`Healer` now EXECUTES `VISIBLE`'s `WAIT_AND_RETRY`/`NO_SAFE_RECOVERY` (Sprint 8, Option A narrowed) — unit-verified, not yet live-verified. `RECEIVES_EVENTS` remains proposal-only, still rejected.** |
