...
| 4 | Selector healer vs UI automation healer scope | 🟡 Narrowed after empirical falsification | Sprint 2 assumed `detached_from_dom` would dominate real-world failures, based on Selenium/`ElementHandle`-style Salesforce Lightning experience. Sprint 6A built a real reproduction mechanism and ran four escalating attempts (200-800ms → deterministic `mousedown` trigger) — none reproduced against Playwright's `Locator` API. Root cause: `Locator.click()` retries automatically on mid-action detachment; this is a documented architectural property, not incidental. Real "not attached" errors in `Locator`-based suites trace to `ElementHandle` misuse, a narrow version-specific edge case, or a sub-frame actionability-to-dispatch race fixed via locator anchoring, not selector/retry healing. `FailureType` enum unchanged; Sprint 6B redirected to `NOT_VISIBLE`/`TIMEOUT_WAITING` (decision pending). See `LEARNINGS.md` Sprint 6A conclusion |
| 5 | No failure classifier component | 🟡 Partially resolved | `classify_playwright_error()` exists and works for `SELECTOR_NOT_FOUND` (Sprint 2/4) and, pending live verification, `DETACHED_FROM_DOM` (Sprint 6A); still no strategy for `NOT_VISIBLE`/`TIMEOUT_WAITING` |
